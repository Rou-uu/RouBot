...
class music_cog(commands.Cog):
    ...
    async def play_next(self):
        if len(self.music_queue) > 0:
            ...
            song = data['url']
            self.voiceChannel.play(discord.FFmpegPCMAudio(song, executable="ffmpeg.exe", **self.FFMPEG_OPTIONS),
    ...
    async def play_music(self, ctx):
        if len(self.music_queue) > 0:
            ...
            if self.voiceChannel == None or not self.voiceChannel.is_connected():
                voice_chan = ctx.author.voice
                if voice_chan is None:
                    print("nadota")

                print("Attempting to connect to " + str(voice_chan.channel))
                try:
                    self.voiceChannel = await voice_chan.channel.connect()
